...
class ImageFlowWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.selected_label = self._stat(stats_l, "☑", "Selected", "0")
    ...
    def _update_timeline(self):
        ...
        for slot, b in enumerate(self.thumb_buttons):
            ...
            b.setProperty("selected", valid and idx in self.selected)
            b.style().unpolish(b)
            b.style().polish(b)
            b.update()
    ...
    def toggle_current_selection(self):
        ...
        if self.index in self.selected:
            ...
            message = f"Deselected image {self.index + 1}"
        else:
            self.selected.add(self.index)
            message = f"Selected image {self.index + 1}"
        self._update_stats()
        self._update_timeline()
        self.statusBar().showMessage(
            f"{message}  |  Selected: {len(self.selected)}", 1800
        )
    ...
    def _update_stats(self):
        self.selected_label.setText(str(len(self.selected)))
    ...
    def copy_current(self):
        ...
        if self.selected:
            items = [(idx, self.paths[idx]) for idx in sorted(self.selected)
                     if 0 <= idx < len(self.paths)]
            if items:
                self._copy_selected(items, dst)
                return

        source = Path(self.paths[self.index])
        self._file_copy(source, dst)

    def _copy_selected(self, items, dst):
        ...
        def op():
            ...
            for idx, path_string in items:
                ...
                # Preserve the existing ImageFlow behavior: copied source is
                # marked with '-selected' in the source folder.
    ...
    def _batch_copy_done(self, results, current_index):
        ...
        for idx, target, selected_source in results:
            if 0 <= idx < len(self.paths):
                self.paths[idx] = str(selected_source)

        self.copied_count += len(results)
        self.selected.clear()
        self._update_stats()
        self._update_timeline()
        ...
        if 0 <= current_index < len(self.paths):
    ...
    def _delete_done(self, source, recycle_target, old_index):
        ...
        # Remove the deleted image from the multi-selection and shift indexes
        # after it so remaining selections stay attached to the same images.
